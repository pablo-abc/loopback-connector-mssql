...
function getDataTypesFromModel(sql, models) {
  ...
  var affectedParameters = sql.match(/\((\[[^\]]+\](, *\[[^\]]+\])*)\)/)[1]
    .replace(/\[/g, '')
    .replace(/\]/g, '')
    .replace(/ /g, '')
    .toLowerCase()
  ...
  var affectedParameterTypes = affectedParameters.map(parameter => {
    return {
      dataType: models[affectedModel].properties[parameter].mssql ?
        models[affectedModel].properties[parameter].mssql.dataType : null,
      typeOptions: [
        models[affectedModel].properties[parameter].length ||
          models[affectedModel].properties[parameter].precision || null,
        models[affectedModel].properties[parameter].scale || null,
      ],
    };
  });
...
function setDataTypesToRequest(params, request, affectedParameterTypes) {
  if (Array.isArray(params) && params.length > 0) {
    for (var i = 0, n = params.length; i < n; i++) {
      if (affectedParameterTypes[i].dataType) {
        request.input(
          'param' + (i + 1),
          mssql[affectedParameterTypes[i].dataType](...affectedParameterTypes[i].typeOptions),
          params[i]
        );
      } else {
