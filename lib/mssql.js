...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  var affectedParameterTypes = affectedParameters.map(parameter => {
    return {
      dataType: this._models[affectedModel].properties[parameter].mssql ?
        this._models[affectedModel].properties[parameter].mssql.dataType : null,
      typeOptions: [
        this._models[affectedModel].properties[parameter].length ||
          this._models[affectedModel].properties[parameter].precision || null,
        this._models[affectedModel].properties[parameter].scale || null,
      ],
    };
  });
  // Convert (?) to @paramX
  ...
  if (Array.isArray(params) && params.length > 0) {
    for (var i = 0, n = params.length; i < n; i++) {
      if (typeof params[i] === 'number' &&
          params[i] % 1 !== 0 &&
          affectedParameterTypes.dataType) {
        // Float number
        request.input(
          'param' + (i + 1),
