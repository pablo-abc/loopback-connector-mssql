...
function getDataTypesFromModel(sql, models) {
  var affectedModel = sql.match(/\[dbo]\.\[([^\]]+)/);
  affectedModel = affectedModel ? affectedModel[1] : '';
  var affectedParameters = /(^SELECT)||(^EXEC)||(^DELETE)/.test(sql) ?
    [] : sql.match(/(\((\[[^\]]+\](, *\[[^\]]+\])*)\))|(SET \[[^\]]*\]=@param[0-9]+(,\[[^\]]*\]=@param[0-9]+)*)/)[0]
      .match(/\[[^\]]+\]/g)
      .toString()
      .replace(/\[/g, '')
      .replace(/\]/g, '')
  ...
  var affectedParameterTypes = affectedParameters.map(parameter => {
    const properties = models[affectedModel].properties;
    var foundParam = Object.keys(properties).find(property => {
      return properties[property].mssql.columnName.toLowerCase() === parameter.toLowerCase();
    });
    return {
      dataType: models[affectedModel].properties[foundParam].mssql ?
        models[affectedModel].properties[foundParam].mssql.dataType : null,
      typeOptions: [
        models[affectedModel].properties[foundParam].length ||
          models[affectedModel].properties[foundParam].precision || null,
        models[affectedModel].properties[foundParam].scale || null,
      ],
    };
  });
  ...
  return {
    affectedModel,
    affectedParameters,
    affectedParameterTypes,
  };
};

function setDataTypesToRequest(params, request, affectedParameterTypes) {
  if (Array.isArray(params) && params.length > 0) {
    for (var i = 0, n = params.length; i < n; i++) {
      if (affectedParameterTypes.length > 0 &&
          affectedParameterTypes.length < i &&
          affectedParameterTypes[i].dataType) {
        const foundType = Object.keys(mssql).find(type => {
          return type.toLowerCase() === affectedParameterTypes[i].dataType.toLowerCase();
        });
        request.input(
          'param' + (i + 1),
          mssql[foundType](...affectedParameterTypes[i].typeOptions),
          params[i]
        );
      } else {
...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  // Getting info from the sql string before parameterize
  var {affectedParameterTypes} = getDataTypesFromModel(sql, this._models);
  // Convert (?) to @paramX
  ...
  setDataTypesToRequest(params, request, affectedParameterTypes).query(sql, innerCB);
  // if (Array.isArray(params) && params.length > 0) {
  //   for (var i = 0, n = params.length; i < n; i++) {
