...
MsSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  var affectedParameters = sql.match(/\((\[[^\]]+\](, *\[[^\]]+\])*)\)/)[1]
    ...
    .toLowerCase()
    .split(',');
  var affectedParametersTypes = affectedParameters.map(parameter => {
    return {
      dataType: this._models[affectedModel].properties[parameter].mssql.dataType,
      dataPrecision: this._models[affectedModel].properties[parameter].mssql.dataPrecision || null,
      dataScale: this._models[affectedModel].properties[parameter].mssql.dataScale || null,
    };
  });
  // Convert (?) to @paramX
  ...
  if (Array.isArray(params) && params.length > 0) {
    for (var i = 0, n = params.length; i < n; i++) {
      if (typeof params[i] === 'number' &&
        params[i] % 1 !== 0) {
        // Float number
        switch (affectedParametersTypes[i].dataType) {
          case 'real':
          ...
          case 'decimal':
            request.input(
              'param' + (i + 1),
              mssql.Decimal(affectedParametersTypes[i].dataPrecision, affectedParametersTypes[i].dataScale),
              params[i]);
            break;
          case 'float':
            request.input('param' + (i + 1), mssql.Float, params[i]);
            break;
        }
      } else {
